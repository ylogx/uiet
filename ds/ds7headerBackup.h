...
void push(NODEPTR tempStart,int x){
  ...
  q->next=top;//NOTE next points to the previous item on stack
  //traverse to find last node
  //tempStart=top(tempStart);
  top=q;//prev last node containing null now points to new node
}//end insert after
...
int pop(NODEPTR *tempTop){
  ...
  if(**tempTop->next==NULL){
    ...
    int dataPoped;
    NODEPTR temp;
    dataPoped=*tempTop->info;
    //printf("\n%d\n",(tempTop)->info);  //debug
    //printf("%d",ptr->info);   //debug
    temp=*tempTop;
    *tempTop=*tempTop->next;  //NOTE modifies global variable !
