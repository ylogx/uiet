...
int getNode(void){
  ...
  int p;
  if (node[avail].next==(-1)){   //no nodes available
    printf("\nOverflow");
    exit(1);
  }//end if
  else if (node[avail].next==NULL){
    return(avail);
  }
  else{
    avail+=1;
...
int main(){
  ...
  for(i=0;i<NUMNODES-1;i++)
    node[i].next=i+1;  //each node points to next node
  node[NUMNODES-1].next=(-1);//-1 is assumed null
