...
/*
 *
 * name: dldfs - Depth Limited DFS
 * @param adjacency matrix, size of matrix, source<1-num>, dest(if 0, traverse graph)
 * @return none
 *
 */
void dldfs(int **adjm,int num,int src,int dest){
    cout << "--------Depth Limited DFS ------"<<endl;
    enum label_list{NEW, MARKED, DONE} label[num];
    //enum label_list label[num];
    for(int i=1;i<=num;++i)
        label[i]=NEW;
 ...
 *
 */
void iddfs(int **adjm,int num,int src,int dest){
    cout << "--------Iterative Deepening DFS ------"<<endl;
    enum label_list{NEW, MARKED, DONE} label[num];
    //enum label_list label[num];
    for(int i=1;i<=num;++i)
        label[i]=NEW;
